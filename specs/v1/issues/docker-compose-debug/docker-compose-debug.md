...
| `POST /users` | ✅ Working | Admin token required (`user:write`); `roleIds` must be valid UUIDs from roles table; 409 on duplicate email, 401 on missing token, 400 on empty `roleIds` |
